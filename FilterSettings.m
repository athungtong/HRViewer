...
% Last Modified by GUIDE v2.5 19-Mar-2013 22:54:18
...
% collect data for HRV cal (Anurak 3/28/2013);
    Temp1=get(handles.PopMenuHighPass,'string');
    Temp = cell2mat(Temp1(FilterPara{SelCh}.HighValue));
    FilterPara{SelCh}.Highnum = str2num(Temp(1:end-3));
    
    Temp1=get(handles.PopMenuLowPass,'string');
    Temp = cell2mat(Temp1(FilterPara{SelCh}.LowValue));
    FilterPara{SelCh}.Lownum = str2num(Temp(1:end-2));
    
    Temp1=get(handles.PopMenuNotch,'string');
    Temp = cell2mat(Temp1(FilterPara{SelCh}.NotchValue));
    FilterPara{SelCh}.Notchnum = str2num(Temp(1:end-8));
...
if ~isempty(currentCharacter)
    ...
    if currentCharacter==1
        Sel = get(handles.ListBoxCh,'value');
        for i=1:length(handles.FilterPara)
            handles.FilterPara{i}=handles.FilterPara{Sel};
        end
        % Update handles structure
        guidata(hObject, handles);
...
% --- Executes on selection change in PopMenuNotch.
function PopMenuNotch_Callback(hObject, eventdata, handles)
% hObject    handle to PopMenuNotch (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns PopMenuNotch contents as cell array
%        contents{get(hObject,'Value')} returns selected item from PopMenuNotch
